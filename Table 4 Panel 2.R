...
parallelobtainhatbeta=function(it){
  ...
  recovery_procedure=function(n,m,testt,testX,valit,valiX){
    ...
    kappalength=5;
    kappalist=exp(seq(log(0.0000001),log(0.001),length.out=kappalength));
    GCV=array(0,kappalength);
    for(kappait in 1:kappalength){
      kappa=kappalist[kappait];
      #estimate xi
if(eigenvalues[1]/sum(eigenvalues[which(eigenvalues>0)])>0.85){#one score
      hatxi1=array(0,n);
      for(i in 1:n){
        numeritor=0;
        dominant=0;
...
if((eigenvalues[1]+eigenvalues[2])/sum(eigenvalues[which(eigenvalues>0)])>0.85){#two eigen
    hatxi=array(0,c(n,2));
    for(i in 1:n){
      A=array(0,c(2,2));V=array(0,2);
      for(j in 1:m_sparse){
        V[1]=V[1]+(fitEX[i,j]-X[i,j])*eigenvectors[ceiling(t[i,j]*l),1]/m_sparse;
        V[2]=V[2]+(fitEX[i,j]-X[i,j])*eigenvectors[ceiling(t[i,j]*l),2]/m_sparse;
        A[1,1]=A[1,1]+eigenvectors[ceiling(t[i,j]*l),1]*eigenvectors[ceiling(t[i,j]*l),1]/m_sparse;
        A[1,2]=A[1,2]+eigenvectors[ceiling(t[i,j]*l),2]*eigenvectors[ceiling(t[i,j]*l),1]/m_sparse;
        A[2,1]=A[2,1]+eigenvectors[ceiling(t[i,j]*l),1]*eigenvectors[ceiling(t[i,j]*l),2]/m_sparse;
        A[2,2]=A[2,2]+eigenvectors[ceiling(t[i,j]*l),2]*eigenvectors[ceiling(t[i,j]*l),2]/m_sparse;
      }
      for(o in 1:l){
        V[1]=V[1]+(EXp[o]+hatbeta*EX[o])*(eigenvectorsp[o,1]+hatbeta*eigenvectors[o,1])*kappa/l;
        V[2]=V[2]+(EXp[o]+hatbeta*EX[o])*(eigenvectorsp[o,2]+hatbeta*eigenvectors[o,2])*kappa/l;
        A[1,1]=A[1,1]+(eigenvectorsp[o,1]+hatbeta*eigenvectors[o,1])*(eigenvectorsp[o,1]+hatbeta*eigenvectors[o,1])*kappa/l;
        A[1,2]=A[1,2]+(eigenvectorsp[o,2]+hatbeta*eigenvectors[o,2])*(eigenvectorsp[o,1]+hatbeta*eigenvectors[o,1])*kappa/l;
        A[2,1]=A[2,1]+(eigenvectorsp[o,1]+hatbeta*eigenvectors[o,1])*(eigenvectorsp[o,2]+hatbeta*eigenvectors[o,2])*kappa/l;
        A[2,2]=A[2,2]+(eigenvectorsp[o,2]+hatbeta*eigenvectors[o,2])*(eigenvectorsp[o,2]+hatbeta*eigenvectors[o,2])*kappa/l;
      }
      hatxi[i,]=-solve(A)%*%V;
    }
    #fit X on t
    ...
    for(i in 1:n){
      for(j in 1:m){
        fitX[i,j]=fitEX[i,j]+hatxi[i,1]*eigenvectors[ceiling(t[i,j]*l),1]+hatxi[i,2]*eigenvectors[ceiling(t[i,j]*l),2];
      }
    }
    GCV[kappait]=sum((fitX[,]-valiX[,])**2)/(n*m);
}#two eigen
  ...
  if(sum(GCV)==0){return(-1);}
    return(kappalist[which.min(GCV)]);
  }#end of select kappa
  #select kappa
  kappa=recovery_procedure(n/2,m_sparse,t_sparse[1:(n/2),],Xcurrent_sparse[1:(n/2),],t_sparse[(n/2+1):n,],Xcurrent_sparse[(n/2+1):n,])
  if(kappa==-1){return(-1);}
  ...
  h_length=5;
  h_candidate=exp(seq(log(0.3),log(0.6),length.out=h_length));
  h_EX=select_h_one_dimen_one_poly(n,m_sparse,t_sparse,Xcurrent_sparse,h_length,h_candidate,0);  
  h_EXp=select_h_one_dimen_one_poly(n,m_sparse,t_sparse,Xcurrent_sparse,h_length,h_candidate,1);
  l=20;
  H=min(max(h_EXp,h_EX),0.45);
  betatanchor=seq(H,1-H,length.out=l);
  # EX
  EX=array(0,l);
  for(o in 1:l){
    EX[o]=one_dimen_one_poly(betatanchor[o],n,m_sparse,t_sparse,Xcurrent_sparse,h_EX,0);
  }
  # EXp
  EXp=array(0,l);
  for(o in 1:l){
    EXp[o]=one_dimen_one_poly(betatanchor[o],n,m_sparse,t_sparse,Xcurrent_sparse,h_EXp,1);
  }
  # hatbeta
  hatbeta=-sum(EX*EXp)/sum(EX*EX);
  ...
  for(i in 1:n){
    for(o in 1:l){
      realX[i,o]=Xcurrent[i,o*(m/l)];
    }
  }
  centerX=array(0,c(n,m_sparse));
  fitEX=array(0,c(n,m_sparse));
  for(i in 1:n){
    for(j in 1:m_sparse){
      fitEX[i,j]=one_dimen_one_poly(t_sparse[i,j],n,m_sparse,t_sparse,Xcurrent_sparse,h_EX,0);
      centerX[i,j]=Xcurrent_sparse[i,j]-fitEX[i,j];
    }
  }
  EX=array(0,l);
  for(o in 1:l){
    EX[o]=one_dimen_one_poly(tanchor[o],n,m_sparse,t_sparse,Xcurrent_sparse,h_EX,0);
  }
  EXp=array(0,l);
  for(o in 1:l){
    EXp[o]=one_dimen_one_poly(tanchor[o],n,m_sparse,t_sparse,Xcurrent_sparse,h_EXp,1);
  }
  C=array(0,c(l,l));
  h_C=select_h_two_dimen_one_poly(n,m_sparse,t_sparse,centerX,h_length,h_candidate,0);
  for(o1 in 1:l){
    for(o2 in 1:l){
      C[o1,o2]=two_dimen_one_poly(tanchor[o1],tanchor[o2],n,m_sparse,t_sparse,centerX,h_C,0);
    }
  }
  temp=eigen(C,symmetric=TRUE)
  eigenvalues=temp$values; #eigenvalues[1]/sum(eigenvalues[which(eigenvalues>0)])>0.85
  ...
  Cp=array(0,c(l,l));
  h_Cp=select_h_two_dimen_one_poly(n,m_sparse,t_sparse,centerX,h_length,h_candidate,1);
  for(o1 in 1:l){
    for(o2 in 1:l){
      Cp[o1,o2]=two_dimen_one_poly(tanchor[o1],tanchor[o2],n,m_sparse,t_sparse,centerX,h_Cp,1);
    }
  }
  eigenvectorsp=array(0,c(l,10)); #the derivative of eigenvector
  for(k in 1:10){
...
###############################
#estimate xi_{i,1}  
if(eigenvalues[1]/sum(eigenvalues[which(eigenvalues>0)])>0.85){# one eigen
  hatxi1=array(0,n);
  for(i in 1:n){
    numeritor=0;
    dominant=0;
    for(j in 1:m_sparse){
      numeritor=numeritor+(fitEX[i,j]-Xcurrent_sparse[i,j])*eigenvectors[ceiling(t_sparse[i,j]*l),1];
      dominant=dominant+eigenvectors[ceiling(t_sparse[i,j]*l),1]*eigenvectors[ceiling(t_sparse[i,j]*l),1];
    }
    numeritor=numeritor*l/(m_sparse*kappa);
    dominant=dominant*l/(m_sparse*kappa);
    for(o in 1:l){
      numeritor=numeritor+(EXp[o]+hatbeta*EX[o])*(eigenvectorsp[o,1]+hatbeta*eigenvectors[o,1]);
      dominant=dominant+(eigenvectorsp[o,1]+hatbeta*eigenvectors[o,1])*(eigenvectorsp[o,1]+hatbeta*eigenvectors[o,1]);
    }
  ...
  return(sum((recoveryX[,]-realX[,])**2)/sum((realX[,])**2));
}# one eigen
...
if((eigenvalues[1]+eigenvalues[2])/sum(eigenvalues[which(eigenvalues>0)])>0.85){# two eigen
  ...
  for(i in 1:n){
    A=array(0,c(2,2));V=array(0,2);
    for(j in 1:m_sparse){
      V[1]=V[1]+(fitEX[i,j]-Xcurrent_sparse[i,j])*eigenvectors[ceiling(t_sparse[i,j]*l),1]/m_sparse;
      V[2]=V[2]+(fitEX[i,j]-Xcurrent_sparse[i,j])*eigenvectors[ceiling(t_sparse[i,j]*l),2]/m_sparse;
      A[1,1]=A[1,1]+eigenvectors[ceiling(t_sparse[i,j]*l),1]*eigenvectors[ceiling(t_sparse[i,j]*l),1]/m_sparse;
      A[1,2]=A[1,2]+eigenvectors[ceiling(t_sparse[i,j]*l),2]*eigenvectors[ceiling(t_sparse[i,j]*l),1]/m_sparse;
      A[2,1]=A[2,1]+eigenvectors[ceiling(t_sparse[i,j]*l),1]*eigenvectors[ceiling(t_sparse[i,j]*l),2]/m_sparse;
      A[2,2]=A[2,2]+eigenvectors[ceiling(t_sparse[i,j]*l),2]*eigenvectors[ceiling(t_sparse[i,j]*l),2]/m_sparse;
    }
    for(o in 1:l){
      V[1]=V[1]+(EXp[o]+hatbeta*EX[o])*(eigenvectorsp[o,1]+hatbeta*eigenvectors[o,1])*kappa/l;
      V[2]=V[2]+(EXp[o]+hatbeta*EX[o])*(eigenvectorsp[o,2]+hatbeta*eigenvectors[o,2])*kappa/l;
  ...
  for(i in 1:n){
    for(o in 1:l){
      recoveryX[i,o]=EX[o]+hatxi[i,1]*eigenvectors[o,1]+hatxi[i,2]*eigenvectors[o,2];
    }
  }
  return(sum((recoveryX[,]-realX[,])**2)/sum((realX[,])**2));
}# two eigen
...
relaerror=array(0,40);
for(m_sparse in c(15:37)){
  itermax=100;
  result_recoveryMSE=array(0,itermax);
  #parallel
  closeAllConnections();
  closeAllConnections();
  cl <- makeCluster(100);
  ...
  cat(m_sparse,round(mean(result_recoveryMSE),4),round(sd(result_recoveryMSE)/sqrt(length(result_recoveryMSE)),4),"\n");
  relaerror[m_sparse]=round(mean(result_recoveryMSE),4)
}  

#plot
library("ggplot2")
data=as.data.frame(cbind(relaerror[15:37],c(15:37)))
ggplot(data)+geom_line(aes(x=V2,y=V1),color="black",size=0.5)+
