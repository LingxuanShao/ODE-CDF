    ...
    for(m in c(3,5,7,9,11)){
      ...
      while(it<(itermax+1)){
        ...
        if("try-error" %in% class(err)){}
          ...
          PACEMSE=array(0,n);
          for(i in 1:n){
            PACEMSE[i]=sum((temp2[i,]-(2*zeta[i]+zeta[i]*temp$workGrid)*exp(-temp$workGrid))**2)/length(temp$workGrid);
          }
      ...
      cat("(3)do",n,scorenumber,m,round(mean(result_recoveryMSE),4),round(sd(result_recoveryMSE)/sqrt(length(result_recoveryMSE)),4),"\n");
