...
empiricalerror=sum((recoveryX[,]-realX[,])**2)/(n*l)
  
# fit error
fiterror=0
for(i in 1:n){
  for(j in 1:m){
    fiterror=fiterror+(fitEX[i,j]+hatxi1[i]*eigenvectors[ceiling(t[i,j]*l),1]-X[i,j])**2;
  }
}
fiterror=fiterror/(n*m)

# ode error
odeerror=0
for(i in 1:n){
  for(o in 1:l){
    odeerror=odeerror+((EXp[o]+hatxi1[i]*eigenvectorsp[o,1])**2+hatbeta*(EX[o]+hatxi1[i]*eigenvectors[o,1])+8)**2
  }
}
odeerror=odeerror*kappa/(n*l)

return(as.list(c(empiricalerror,kappa,fiterror,odeerror)))
}
...
for(scorenumber in c(1,3,5)){
  for(n in c(100,200)){
    for(m in c(5,10,15,20)){
      ...
      empiricalerror_list=array(0,itermax);
      kappa_list=array(0,itermax);
      fiterror_list=array(0,itermax);
      odeerror_list=array(0,itermax);
      for(it in 1:itermax){
        empiricalerror_list[it]=result[[4*it-3]]
        kappa_list[it]=result[[4*it-2]]
        fiterror_list[it]=result[[4*it-1]]
        odeerror_list[it]=result[[4*it]]
      }
      empiricalerror_list=empiricalerror_list[which(empiricalerror_list>0)]
      kappa_list=kappa_list[which(kappa_list>0)]
      fiterror_list=fiterror_list[which(fiterror_list>0)]
      odeerror_list=odeerror_list[which(odeerror_list>0)]
      
      cat(scorenumber,n,m,
          mean(empiricalerror_list),sd(empiricalerror_list)/sqrt(length(empiricalerror_list)),
          mean(kappa_list),sd(kappa_list)/sqrt(length(kappa_list)),
          mean(fiterror_list),sd(fiterror_list)/sqrt(length(fiterror_list)),
          mean(odeerror_list),sd(odeerror_list)/sqrt(length(odeerror_list)),
          mean(odeerror_list/fiterror_list),sd(odeerror_list/fiterror_list)/sqrt(length(fiterror_list)),
          "\n");
