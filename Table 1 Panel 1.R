...
parallelobtainhatbeta=function(it){
  ...
  #h selection
  h_length=5;
  h_candidate2=exp(seq(log(0.2),log(0.4),length.out=h_length));
...
cat(n,scorenumber,m,round(mean(temp),4),round(sd(temp)/sqrt(itermax),4),"\n");
